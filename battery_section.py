...
class BatteryGraphWidget(QWidget):
    def __init__(self):
        ...
        self.fig, self.ax_battery = plt.subplots(1, 1, figsize=(8, 3), dpi=100)
        ...
        self.ax_battery.grid(axis='both', linestyle='--', color='gray', alpha=0.6)

        # Add to layout and set it
    ...
    def update_graph(self):
        ...
        if battery:  # Check if battery data is available
            ...
            self.ax_battery.plot(self.time_series, self.battery_data, color='green', marker='o')
            self.ax_battery.set_ylim([0, 100])
            self.ax_battery.set_ylabel('Battery (%)')
